...
class MinProgram {
  ...
  async getComponentsEntry(components, instance) {
    ...
    for (const componentPath of componentsPath) {
      ...
      if (components.findIndex(item => item.component === component) < 0) {
        const entryPath = _path.default.posix.join(..._path.default.relative(basePath, component).split(_path.default.sep));
  ...
  async getXMLResource(entries) {
    ...
    for (let i = 0, l = entries.length; i < l; i++) {
      ...
      const extname = _path.default.extname(entry);

      if (targetsName.indexOf(extname) > -1) {
        const source = (0, _fsExtra.readFileSync)(`${(0, _path.resolve)(this.base, entry)}`, 'utf8');
  ...
  addSJSResource(entries, entry, url) {
    const src = _path.default.relative(this.base, _path.default.resolve(this.base, _path.default.dirname(entry), url)).replace(/\\/g, '\/');
  ...
  addCSSResource(entries, entry, url) {
    ...
    if (url[0] === '/') {
      src = _path.default.relative(this.base, _path.default.resolve(this.base, `.${url}`)).replace(/\\/g, '\/');
    } else {
      src = _path.default.relative(this.base, _path.default.resolve(this.base, _path.default.dirname(entry), url)).replace(/\\/g, '\/');
    }
