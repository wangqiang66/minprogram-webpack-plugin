...
class MiniWebpackPlugin extends _MinProgram.default {
          ...
          if (chunk.hasEntryModule()) {
            ...
            for (const module of chunk.getModules()) {
              if (!module.isEntryModule()) {
                ...
                let relPath = _utils.default.getDistPath(resourcePath);

                let chunkName = chunk.name + '.js';

                _utils.default.setMapValue(DEPS_MAP, relPath, chunkName);
  ...
  setCompilation(compilation) {
    ...
    const globalVar = Targets[target.name].global; // compilation.mainTemplate.hooks.assetPath.tap('MinWebpackPlugin', (path) => {
    ...
    compilation.chunkTemplate.hooks.render.tap('MinWebpackPlugin', (core, chunk) => {
      ...
      if (absoluteEntryPath.indexOf(chunk.resource) >= 0) {
        const relativePath = (0, _path.relative)((0, _path.dirname)(name), `./webpack-require`);
