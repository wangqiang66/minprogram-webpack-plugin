...
export class MiniWebpackPlugin extends MinProgram {
  constructor(options) {
  ...
  addScriptEntry(compiler, entry, name) {
    compiler.hooks.make.tapAsync('MiniWebpackPlugin', (compilation, callback) => {
      if (compiler.options.target.name === Targets.Wechat.name) {
        name = name.replace(/node_modules/, 'miniprogram_npm')
      }
  ...
  async compileAssets(compiler) {
    ...
    // 处理相关的依赖
    await this.getXMLResource(entries)
    await this.getCSSResourceByEntries(entries)
  ...
  setCompilation(compilation) {
    ...
    compilation.mainTemplate.hooks.render.tap('MinWebpackPlugin', (source, chunk, hash, moduleTemplate, dependencyTemplates) => {
      ...
      if (entryResources.findIndex(({ name }) => {
        let tname = name
        if (compilation.options.target.name === Targets.Wechat.name) {
          tname = tname.replace(/node_modules/, 'miniprogram_npm')
        }
        return tname === chunk.name
      }) > -1){
